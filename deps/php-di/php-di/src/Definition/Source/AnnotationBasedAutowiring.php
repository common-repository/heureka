...
use HeurekaDeps\DI\Annotation\Inject;
use HeurekaDeps\DI\Annotation\Injectable;
use HeurekaDeps\DI\Definition\Exception\InvalidAnnotation;
use HeurekaDeps\DI\Definition\ObjectDefinition;
use HeurekaDeps\DI\Definition\ObjectDefinition\MethodInjection;
use HeurekaDeps\DI\Definition\ObjectDefinition\PropertyInjection;
use HeurekaDeps\DI\Definition\Reference;
use HeurekaDeps\Doctrine\Common\Annotations\AnnotationRegistry;
use HeurekaDeps\Doctrine\Common\Annotations\Reader;
use HeurekaDeps\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use HeurekaDeps\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'HeurekaDeps\\DI\\Annotation\\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'HeurekaDeps\\DI\\Annotation\\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('HeurekaDeps\\DI\\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'HeurekaDeps\\DI\\Annotation\\Injectable');
