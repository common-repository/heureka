...
namespace HeurekaDeps\PHPStan\PhpDocParser\Parser;

use HeurekaDeps\PHPStan\PhpDocParser\Ast;
use HeurekaDeps\PHPStan\PhpDocParser\Ast\Type\IdentifierTypeNode;
...
class PhpDocParser
{
    ...
    public function parse(TokenIterator $tokens) : Ast\PhpDoc\PhpDocNode
    {
        ...
        } catch (\HeurekaDeps\PHPStan\PhpDocParser\Parser\ParserException $e) {
            $name = '';
            if (\count($children) > 0) {
    ...
    private function parseExtendsTagValue(string $tagName, TokenIterator $tokens) : Ast\PhpDoc\PhpDocTagValueNode
    {
        ...
        throw new \HeurekaDeps\PHPStan\ShouldNotHappenException();
    }
    ...
    private function parseTypeAliasImportTagValue(TokenIterator $tokens) : Ast\PhpDoc\TypeAliasImportTagValueNode
    {
        ...
        if (!$tokens->tryConsumeTokenValue('from')) {
            throw new \HeurekaDeps\PHPStan\PhpDocParser\Parser\ParserException($tokens->currentTokenValue(), $tokens->currentTokenType(), $tokens->currentTokenOffset(), Lexer::TOKEN_IDENTIFIER);
        }
